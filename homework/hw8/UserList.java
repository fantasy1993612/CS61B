...
public class UserList {
    ...
    **/ 
    public static void partition(String sortFeature, CatenableQueue<User> qUnsorted, int pivot, 
        ...
        while(!qUnsorted.isEmpty()){   
            item = qUnsorted.dequeue(); 
            if(item == null){
                break;
            }

            u = getPivot(sortFeature,item);
    ...
    **/
    public CatenableQueue<CatenableQueue<User>> makeQueueOfQueues(){
        //Replace with solution.
        CatenableQueue<CatenableQueue<User>> queues = new CatenableQueue<CatenableQueue<User>>() ;
        while(userQueue.size() > 0){
            User u = userQueue.dequeue();
            CatenableQueue<User> usrq = new CatenableQueue<User>();
            usrq.enqueue(u);
            queues.enqueue(usrq); 
        }
        return queues;
    ...
    **/
    public static CatenableQueue<User> mergeTwoQueues(String sortFeature, CatenableQueue<User> q1, CatenableQueue<User> q2){
        ...
        CatenableQueue<User> merge = new CatenableQueue<User>();
        int i = 0;

        while(q1.size() != 0 && q2.size() != 0){

            int q1u = getPivot(sortFeature,q1.nth(0));
            int q2u = getPivot(sortFeature,q2.nth(0));
    ...
    **/
    public void mergeSort(String sortFeature){
        ...
        while(queues.size() > 1){
            CatenableQueue<User> q1 = queues.dequeue();
            CatenableQueue<User> q2 = queues.dequeue();
            queues.enqueue(mergeTwoQueues(sortFeature, q1, q2));    
        }
